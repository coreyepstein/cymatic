...
export function useVisualizer(options: UseVisualizerOptions): VisualizerHandle {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const setup = async (): Promise<void> => {
      try {
        // Create + init a renderer, transparently falling back to WebGL if a
        // WebGPU renderer cannot come up at runtime. WebGPU can be selected
        // (because `navigator.gpu` exists) yet still fail in `init()` — e.g. the
        // adapter is null on a GPU-less machine, the adapter is blocklisted, or
        // device creation rejects. The core helper centralizes that policy. A
        // failure that escapes here is a genuine *renderer* error (surfaced
        // as-is), never a microphone error.
        renderer = await createRendererWithFallback(
          canvas as unknown as RenderCanvasLike,
          rendererOptions,
        );
        if (disposed) return;
