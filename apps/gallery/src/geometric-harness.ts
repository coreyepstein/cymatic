/**
 * V2-10 geometric cinematic verification harness (real browser, real GPU pass —
 * no mocks).
 *
 * Drives a geometric preset through the public preset lifecycle against the
 ...
 * chain runs; WebGL otherwise). The spec reports the resolved backend.
 */
...
declare global {
  interface Window {
    __geoHarness?: GeometricHarness;
    __geoHarnessReady?: boolean;
    __geoHarnessError?: string;
  }
}
...
async function boot(): Promise<void> {
  ...
  if (!canvas) throw new Error("geometric-harness: canvas #c missing");
  ...
  const harness: GeometricHarness = {
    ...
    renderWith: async (
      ...
      if (!preset) throw new Error("geometric-harness: no preset selected");
  ...
  window.__geoHarness = harness;
  window.__geoHarnessReady = true;
}

boot().catch((err: unknown) => {
  window.__geoHarnessError = err instanceof Error ? err.message : String(err);
  window.__geoHarnessReady = true;
});
